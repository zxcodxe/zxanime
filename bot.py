...
class Bot(Client):
    ...
    def get_current_settings(self):
        """Returns current settings for saving."""
        return {
            "admins": self.admins,
    ...
    async def start(self):
        ...
        if saved_settings:
            self.LOGGER(
                ...
                "Found saved settings in database. "
                "Using START messages from setup.json."
            )

            # Do NOT merge saved MongoDB messages.
            # START and START_PHOTO must always come from setup.json.

            self.admins = saved_settings.get(
                "admins",
                self.admins
            )
            ...
            self.protect = saved_settings.get(
                "protect",
                self.protect
            )
            ...
            self.LOGGER(
                ...
                "Using initial config from setup.json."
            )
            ...
            for channel in self.fsub:
                try:
                    ...
                    self.LOGGER(
                        ...
                        f"Bot can't Export Invite link from "
                        f"Force Sub Channel {channel[0]}! Error: {e}"
                    )
        ...
        try:
            ...
            self.LOGGER(
                ...
                f"and Double check the database channel value. "
                f"Current Value: {self.db}"
            )
